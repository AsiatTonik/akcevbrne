    ...
    <div class="events-grid">
        <?php foreach ($events_page as $event): ?>
            <div class="event-card">
                <img src="<?= $event['attributes']['first_image'] ?? 'obrazek.png' ?>" alt="Obrázek akce" class="event-image">
                <div class="event-info">
                <h2><?= $event['attributes']['name'] ?? 'Název akce' ?></h2>
                <p>
                <?= !empty($event['attributes']['text']) ? substr($event['attributes']['text'], 0, 80) . '...' : 'Popis akce není k dispozici.' ?>
                </p>                                           
                <a href="<?= base_url('udalost/' . $event['attributes']['ID']) ?>" class="more-button">VÍCE</a>                
                </div>
            </div>
        <?php endforeach; ?>
    </div>
